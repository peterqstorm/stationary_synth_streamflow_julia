...
function main()
    ...
    for i = 1:length(files)
        ...
        CSV.write(file_name*"-daily.csv", Tables.table(q_nx365), delim=" ", writeheader=false)
        CSV.write(file_name*"-monthly.csv", Tables.table(Qfinal_monthly[i]), delim=" ", writeheader=false)
        println("Successfully cleaned daily and monthly files for " * ff[1:length(ff) - 10])
    end
end
